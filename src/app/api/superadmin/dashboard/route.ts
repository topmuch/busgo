...
export async function GET() {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    mrrHistory.push({ month: label, mrr: paid.reduce((s, inv) => s + inv.amount, 0) });
  }
  ...
  for (const inv of failed) {
    alerts.push({ type: "payment_failed", message: `Paiement échoué — ${inv.number}`, tenantName: inv.tenant.name, severity: "error" });
  }

  const overdue = await db.invoice.findMany({ where: { status: "pending", dueDate: { lt: now } }, include: { tenant: { select: { name: true } } } });
  for (const inv of overdue) {
    alerts.push({ type: "invoice_overdue", message: `Facture en retard — ${inv.number}`, tenantName: inv.tenant.name, severity: "warning" });
  }
